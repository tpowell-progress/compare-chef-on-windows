        ...
        if ($rubyBinVersions) {
            ...
            foreach ($rubyDll in $rubyBinVersions) {
                ...
                if ($rubyDll.SHA256 -eq $addedDll.SHA256) {
                    Write-Host "      ✓ IDENTICAL (SHA256 matches)" -ForegroundColor Green
                } else {
                    Write-Host "      ✗ DIFFERENT (SHA256 mismatch)" -ForegroundColor Red
...
if ($addedDlls.Count -eq 0 -and $changedDlls.Count -eq 0) {
    Write-Host "✓ No changes detected - Installing dumpbin did not add or modify any of the tracked DLLs" -ForegroundColor Green
} else {
    if ($addedDlls.Count -gt 0) {
        Write-Host "• Dumpbin installation added $($addedDlls.Count) DLL(s)" -ForegroundColor Yellow
    ...
    if ($changedDlls.Count -gt 0) {
        Write-Host "• Dumpbin installation modified $($changedDlls.Count) existing DLL(s)" -ForegroundColor Red
    }
}
